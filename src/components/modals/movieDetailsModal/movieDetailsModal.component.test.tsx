...
describe('MovieDetailsModal', () => {
  it('should submit new movie', async () => {
    const { getByLabelText, container } = render(getMovieDetailsModal(movieDetailsModalProps));
    ...
    fireEvent.change(releaseDateInput, { target: { value: mockMovie.releaseDate } });
    fireEvent.change(urlInput, { target: { value: mockMovie.url } });
    fireEvent.change(descriptionInput, { target: { value: mockMovie.description } });
  ...
  it('should reset form acter click on `reset` button', () => {
    ...
    fireEvent.change(descriptionInput, { target: { value: 'random description 789' } });
  ...
  it('should show validation errors on blur', async () => {
    ...
    [title, releaseDate, url, description, duration, dropdown].forEach(fireEvent.blur);
    ...
    await waitFor(() => {
      ...
      expect(getByTestId('releaseDate')).toHaveTextContent('please choose a release date.');
      ...
      expect(getByTestId('description')).toHaveTextContent('please add an overview.');
      ...
      expect(getByTestId('genre')).toHaveTextContent('please choose at least one genre.');
    });
  });
